...
interface Props {
	setCourseSelected: Dispatch<SetStateAction<string | null>>
...
export default function CourseModal({
	...
	useEffect(() => {
		void (async () => {
			try {
				const receivedCourse: ReadOneCourseResponse =
					await getCourseById(courseSelected)
	...
	return (
		<section className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
			<article className="z-10 m-auto flex h-full w-2/5 flex-col gap-1 rounded-lg bg-card px-8 py-4 shadow-lg">
				<div className="flex">
					<X
						size={30}
				...
				{loading && (
					<div className="flex h-full w-full flex-col gap-6">
						<div className="h-5 w-full animate-pulse rounded-lg bg-gray-400"></div>
						<div className="h-5 w-full animate-pulse rounded-lg bg-gray-400"></div>
						<div className="h-5 w-full animate-pulse rounded-lg bg-gray-400"></div>
						<div className="aspect-video w-full animate-pulse rounded-lg bg-gray-400"></div>
						<div className="h-20 w-full animate-pulse rounded-lg bg-gray-400"></div>
						<div className="h-20 w-full animate-pulse rounded-lg bg-gray-400"></div>
					</div>
				)}
				{course && (
					<div className="no-scrollbar flex h-auto flex-col space-y-6 overflow-y-scroll">
						<b>{course?.title}</b>
